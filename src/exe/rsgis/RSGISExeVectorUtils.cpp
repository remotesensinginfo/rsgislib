	...
	else
	{
		...
		else if(option == RSGISExeVectorUtils::printattribute)
		{
			...
			try
			{
				...
				//
				/////////////////////////////////////
				inputSHPDS = OGRSFDriverRegistrar::Open(this->inputVector.c_str(), FALSE, NULL);
				if(inputSHPDS == NULL)
				{
					string message = string("Could not open vector file ") + this->inputVector;
					throw RSGISFileException(message.c_str());
		...
		else if(option == RSGISExeVectorUtils::splitbyattribute)
		{
			...
			try
			{
				...
				for(iterAttr = attributeValues->begin(); iterAttr != attributeValues->end(); ++iterAttr)
				{
					...
					//
					/////////////////////////////////////
					outputSHPDS = shpFiledriver->CreateDataSource(outputShpFilePath.c_str(), NULL);
					if( outputSHPDS == NULL )
					{
		...
		else if(option == RSGISExeVectorUtils::polygonsInPolygon)
		{
			...
			try
			{
				...
				{
					string message = string("Could not open cover vector file ") + this->inputCoverVector;
					throw RSGISFileException(message.c_str());
				}
				inputCoverSHPLayer = inputCoverSHPDS->GetLayerByName(coverSHPFileInLayer.c_str());
				if(inputCoverSHPLayer == NULL)
				{
					string message = string("Could not open vector layer ") + coverSHPFileInLayer;
					throw RSGISFileException(message.c_str());
				}
				...
				while( (inCoverFeature = inputCoverSHPLayer->GetNextFeature()) != NULL )
				{
					...
					if( outputSHPDS == NULL )
					{
						string message = string("Could not create vector file ") + outVector;
						throw RSGISVectorOutputException(message.c_str());
					}
					outputSHPLayer = outputSHPDS->CreateLayer(SHPFileOutLayer.c_str(), inputSpatialRef, geometryType, NULL );
					if( outputSHPLayer == NULL )
		...
		else if(option == RSGISExeVectorUtils::splitlargesmall)
		{
			...
			try
			{
				...
				//
				/////////////////////////////////////
				outputSHPDSLarge = shpFiledriver->CreateDataSource(this->outputlarge.c_str(), NULL);
				...
				//
				/////////////////////////////////////
				outputSHPDSSmall = shpFiledriver->CreateDataSource(this->outputsmall.c_str(), NULL);
		...
		else if(option == RSGISExeVectorUtils::removepolygonscontainedwithingeom)
		{
			...
			try
			{
				...
				//
				/////////////////////////////////////
				inputSHPDSGeom = OGRSFDriverRegistrar::Open(this->inputGeometry.c_str(), FALSE);
		...
		else if(option == RSGISExeVectorUtils::vectormaths)
		{
			...
			try
			{
				...
				//
				/////////////////////////////////////
				inputSHPDS = OGRSFDriverRegistrar::Open(this->inputFile.c_str(), FALSE);
				if(inputSHPDS == NULL)
				{
