        ...
        try
		{
			...
			for(int i = 0; i < nYBlocks; i++)
			{
				...
				for(int n = 0; n < this->numOutBands; n++)
				{
					...
					outputRasterBands[n]->RasterIO(GF_Write, 0, rowOffset, width, yBlockSize, outputData[n], width, yBlockSize, GDT_Float32, 0, 0);
				}
			}
            ...
            if(remainRows > 0)
            {
				...
				for(int n = 0; n < this->numOutBands; n++)
				{
					...
					outputRasterBands[n]->RasterIO(GF_Write, 0, rowOffset, width, remainRows, outputData[n], width, remainRows, GDT_Float32, 0, 0);
				}
            }
			std::cout << " Complete.\n";
		}
	...
	*/
		...
		try
		{
            ...
            if(nYBlocks > 0)
            {
                ...
                if(remainRows > 0)
                {
                    ...
                    for(int n = 0; n < this->numOutBands; n++)
                    {
                        outputRasterBands[n]->RasterIO(GF_Write, 0, (nYBlocks*numOfLines), width, remainRows, outputData[n], width, remainRows, GDT_Float32, 0, 0);
                    }
                }
	...
	void RSGISCalcImage::calcImageWindowDataExtent(GDALDataset **datasets, int numDS, std::string outputImage, int windowSize, std::string gdalFormat) throw(RSGISImageCalcException,RSGISImageBandException)
	{
		...
		try
		{
			...
			for(int i = 0; i < numInBands; i++)
			{
				inputData[i] = new float*[windowSize];
				for(int j = 0; j < windowSize; j++)
				{
					inputData[i][j] = (float *) CPLMalloc(sizeof(float)*width);
			...
			zeros = new float[width];
			for(int i = 0; i < width; i++)
			{
				zeros[i] = 0;
			}
			
			int distFromWinCentreX = 0;
			int distFromWinCentreY = 0;
			int feedback = height/10;
			int feedbackCounter = 0;
			int xOffset = 0;
			int yOffset = 0;
			...
			{				
				...
				for(int m = 0; m < windowSize; m++)
				{
					if(m < windowMid)
					{
						distFromWinCentreY = windowMid - m;
						if((i - distFromWinCentreY) < 0)
						{
							for(int n = 0; n < numInBands; n++)
							{
								for(int k = 0; k < width; k++)
						...
						else
						{
							for(int n = 0; n < numInBands; n++)
							{
								xOffset = bandOffsets[n][0];
								yOffset = (bandOffsets[n][1]+(i-distFromWinCentreY));
								inputRasterBands[n]->RasterIO(GF_Read, xOffset, yOffset, width, 1, inputData[n][m], width, 1, GDT_Float32, 0, 0);
							}
						}
					}
					else if(m > windowMid)
					{
						distFromWinCentreY = m - windowMid;
						if((i + distFromWinCentreY) >= height)
						{
							for(int n = 0; n < numInBands; n++)
							{
								for(int k = 0; k < width; k++)
				...
				for(int j = 0; j < width; j++)
				{
					for(int m = 0; m < windowSize; m++)
					{
						for(int k = 0; k < windowSize; k++)
						{
							...
							if(k < windowMid)
							{
								distFromWinCentreX = windowMid - k;
								if((j-distFromWinCentreX) < 0)
								{
									for(int n = 0; n < numInBands; n++)
									{
										inDataBlock[n][m][k] = 0;
								...
								else
								{
									for(int n = 0; n < numInBands; n++)
									{
										inDataBlock[n][m][k] = inputData[n][m][(j-distFromWinCentreX)];
									}
								}
							}
							else if(k > windowMid)
							{
								distFromWinCentreX = k - windowMid;
								if((distFromWinCentreX+j) >= width)
								{
									for(int n = 0; n < numInBands; n++)
									{
										inDataBlock[n][m][k] = 0;
							...
							else
							{
								for(int n = 0; n < numInBands; n++)
								{
									inDataBlock[n][m][k] = inputData[n][m][j];
								}
							}
						}
					}
					...
					this->calc->calcImageValue(inDataBlock, numInBands, windowSize, outDataColumn, extent);
	...
	void RSGISCalcImage::calcImageWithinPolygon(GDALDataset **datasets, int numDS, geos::geom::Envelope *env, geos::geom::Polygon *poly, pixelInPolyOption pixelPolyOption) throw(RSGISImageCalcException,RSGISImageBandException)
	{
		...
		try
		{
			...
			{				
				...
				for(int n = 0; n < this->numOutBands; n++)
				{
					outputRasterBands[n]->RasterIO(GF_Read, bandOffsets[n][0], (bandOffsets[n][1]+i), width, 1, outputData[n], width, 1, GDT_Float32, 0, 0);
				}
				...
				for(int n = 0; n < this->numOutBands; n++)
				{
					outputRasterBands[n]->RasterIO(GF_Write, bandOffsets[n][0], (bandOffsets[n][1]+i), width, 1, outputData[n], width, 1, GDT_Float32, 0, 0);
				}
			}
			if (height > 100) 
			{
