...
def test_get_rat_columns():
    ...
    if len(rat_columns) != len(ref_columns):
        cols_match = False
        print("Number of columns is different: {} != {}".format(len(rat_columns), len(ref_columns)))
...
def test_populate_rat_with_stats(tmp_path):
    ...
    band_stats = list()
    band_stats.append(rsgislib.rastergis.BandAttStats(band=1, min_field='b1Min', max_field='b1Max', sum_field='b1Sum', std_dev_field='b1StdDev', mean_field='b1Mean'))
    band_stats.append(rsgislib.rastergis.BandAttStats(band=2, min_field='b2Min', max_field='b2Max', sum_field='b2Sum', std_dev_field='b2StdDev', mean_field='b2Mean'))
    band_stats.append(rsgislib.rastergis.BandAttStats(band=3, min_field='b3Min', max_field='b3Max', sum_field='b3Sum', std_dev_field='b3StdDev', mean_field='b3Mean'))
    band_stats.append(rsgislib.rastergis.BandAttStats(band=4, min_field='b4Min', max_field='b4Max', sum_field='b4Sum', std_dev_field='b4StdDev', mean_field='b4Mean'))
    ...
    vars_to_test = ['Histogram', 'b1Min', 'b2Max', 'b3Sum', 'b4StdDev', 'b1Mean', 'b2Mean', 'b3Mean', 'b4Mean']
    vars_eq_vals = True
