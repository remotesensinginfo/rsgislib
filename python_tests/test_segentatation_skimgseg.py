...
def test_perform_felsenszwalb_segmentation(tmp_path):
    ...
    rsgislib.segmentation.skimgseg.perform_felsenszwalb_segmentation(input_img, out_clumps_img, gdalformat='KEA', no_data_val=0, tmp_dir=tmp_dir, calc_stats=True, use_pca=True, n_pca_bands=3, pca_pxl_sample=100, scale=1, sigma=0.8, min_size=20)
...
def test_perform_quickshift_segmentation(tmp_path):
    ...
    if not os.path.exists(tmp_dir):
        os.mkdir(tmp_dir)
    out_clumps_img = os.path.join(tmp_path, "sen2_20210527_aber_clumps.kea")
    rsgislib.segmentation.skimgseg.perform_quickshift_segmentation(input_img, out_clumps_img, gdalformat='KEA', no_data_val=0, tmp_dir=tmp_dir, calc_stats=True, use_pca=True, pca_pxl_sample=100, ratio=1.0, kernel_size=5, max_dist=10, sigma=0, convert_to_lab=True, random_seed=42)
    assert os.path.exists(out_clumps_img)
...
def test_perform_slic_segmentation(tmp_path):
    ...
    if not os.path.exists(tmp_dir):
        os.mkdir(tmp_dir)
    out_clumps_img = os.path.join(tmp_path, "sen2_20210527_aber_clumps.kea")
    rsgislib.segmentation.skimgseg.perform_slic_segmentation(input_img, out_clumps_img, gdalformat='KEA', no_data_val=0, tmp_dir=tmp_dir, calc_stats=True, use_pca=True, n_pca_bands=3, pca_pxl_sample=100, n_segments=100, compactness=10.0, max_iter=10, sigma=0, spacing=None, convert_to_lab=None, enforce_connectivity=True, min_size_factor=0.5, max_size_factor=3, slic_zero=False)
    assert os.path.exists(out_clumps_img)
