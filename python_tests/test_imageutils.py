...
def test_setEnvVarsLZWGTiffOuts_BigTiff():
    ...
    assert os.environ["RSGISLIB_IMG_CRT_OPTS_GTIFF"] == "TILED=YES:COMPRESS=LZW:BIGTIFF=YES"
...
def test_getRSGISLibDataTypeFromImg():
    ...
    assert rsgislib.imageutils.getRSGISLibDataTypeFromImg(input_img) == rsgislib.TYPE_16UINT
...
def test_getImageBBOX():
    ...
    assert (abs(bbox[0] - 257060.59483340546) < 0.0001) and \
           (abs(bbox[1] - 266370.59483340546) < 0.0001) and \
           (abs(bbox[2] - 276258.7167059921) < 0.0001)  and \
           (abs(bbox[3] - 285728.7167059921) < 0.0001)
