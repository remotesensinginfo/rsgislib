...
def test_subset_roi_contained(tmp_path):
    ...
    vec_lyr = "roi_subset"
    output_img = os.path.join(tmp_path, "sen2_20210527_aber_subset.tif")

    rsgislib.imageutils.subset(
        input_img, vec_file, vec_lyr, output_img, "GTIFF", rsgislib.TYPE_16UINT
    )
...
def test_pop_img_stats(tmp_path):
    ...
    input_ref_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset.tif")
    input_img = os.path.join(tmp_path, "sen2_20210527_aber_subset.tif")
...
def test_pop_thmt_img_stats(tmp_path):
    ...
    input_ref_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset_vldmsk.tif")
    input_img = os.path.join(tmp_path, "sen2_20210527_aber_subset_vldmsk.tif")
    copy2(input_ref_img, input_img)
...
def test_get_gdal_format_name():
    ...
    input_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset.tif")
    assert "GTIFF" == rsgislib.imageutils.get_gdal_format_name(input_img)
...
def test_copy_proj_from_img(tmp_path):
    ...
    input_utm_img = os.path.join(DATA_DIR, "sen2_20210527_aber_utm30n.tif")
    input_ref_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset.tif")
    input_img = os.path.join(tmp_path, "sen2_20210527_aber_subset.tif")
    copy2(input_ref_img, input_img)
...
def test_resample_img_to_match(tmp_path):
    ...
    input_utm_img = os.path.join(DATA_DIR, "sen2_20210527_aber_utm30n.tif")
    input_osgb_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")

    output_img = os.path.join(tmp_path, "output_img.tif")
    rsgislib.imageutils.resample_img_to_match(
        input_utm_img, input_osgb_img, output_img, "GTIFF"
    )
...
def test_reproject_image(tmp_path):
    ...
    output_img = os.path.join(tmp_path, "output_img.tif")
    rsgislib.imageutils.reproject_image(input_osgb_img, output_img, wkt_file, gdalformat="GTIFF")
...
def test_gdal_warp(tmp_path):
    ...
    rsgislib.imageutils.gdal_warp(
        input_img, output_img, 4326, gdalformat="GTIFF", use_multi_threaded=False
    )
...
def test_create_img_mosaic(tmp_path):
    ...
    imgs = glob.glob(os.path.join(IMGUTILS_DATA_DIR, "s2_tiles", "*.tif"))
    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.create_img_mosaic(
        imgs, output_img, 0, 0, 1, 0, "GTIFF", rsgislib.TYPE_16UINT
    )
...
@pytest.mark.skipif(
    True,
    reason="Worked with KEA but not with GTIFF - rounding error or something?",
)
def test_include_imgs(tmp_path):
    ...
    input_ref_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.create_copy_img(
        input_ref_img, output_img, 10, 0, "GTIFF", rsgislib.TYPE_16UINT
    )
...
def test_include_imgs_with_overlap(tmp_path):
    ...
    imgs = glob.glob(os.path.join(IMGUTILS_DATA_DIR, "s2_over_tiles", "*.tif"))
...
def test_combine_imgs_to_band(tmp_path):
    ...
    imgs = glob.glob(os.path.join(IMGUTILS_DATA_DIR, "cls_imgs", "*.tif"))
    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.combine_imgs_to_band(
        imgs, output_img, gdalformat="GTIFF", datatype=rsgislib.TYPE_8UINT, no_data_val=0
    )
...
def test_combine_binary_masks(tmp_path):
    ...
    msk_imgs_dict["forest"] = os.path.join(
        IMGUTILS_DATA_DIR, "cls_imgs", "Forest_1226565d33.tif"
    )
    msk_imgs_dict["grass"] = os.path.join(
        IMGUTILS_DATA_DIR, "cls_imgs", "Grass_1226565d33.tif"
    )
    msk_imgs_dict["urban"] = os.path.join(
        IMGUTILS_DATA_DIR, "cls_imgs", "Urban_1226565d33.tif"
    )
    msk_imgs_dict["water"] = os.path.join(
        IMGUTILS_DATA_DIR, "cls_imgs", "Water_1226565d33.tif"
    )

    output_img = os.path.join(tmp_path, "out_img.tif")
    out_lut_file = os.path.join(tmp_path, "out_lut.json")
    rsgislib.imageutils.combine_binary_masks(
        msk_imgs_dict, output_img, out_lut_file, gdalformat="GTIFF"
    )
...
def test_create_tiles(tmp_path):
    ...
    rsgislib.imageutils.create_tiles(
        input_img, out_img_base, 200, 200, 0, False, "GTIFF", rsgislib.TYPE_16UINT, "tif"
    )

    assert len(glob.glob("{}*.tif".format(out_img_base))) == 25
...
def test_create_tiles_multi_core(tmp_path):
    ...
    rsgislib.imageutils.create_tiles_multi_core(
        input_img, out_img_base, 200, 200, "GTIFF", rsgislib.TYPE_16UINT, "tif", 1
    )
...
def test_create_tiles_multi_core_outpath_exp(tmp_path):
    ...
    with pytest.raises(Exception):
        rsgislib.imageutils.create_tiles_multi_core(
            input_img, out_img_base, 200, 200, "GTIFF", rsgislib.TYPE_16UINT, "kea", 1
        )
...
def test_mask_img(tmp_path):
    ...
    input_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    in_msk_img = os.path.join(DATA_DIR, "sen2_20210527_aber_vldmsk.tif")

    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.mask_img(
        input_img, in_msk_img, output_img, "GTIFF", rsgislib.TYPE_16UINT, 0, 0
    )
...
def test_gen_finite_mask(tmp_path):
    ...
    input_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.gen_finite_mask(input_img, output_img, gdalformat="GTIFF")
...
def test_gen_valid_mask(tmp_path):
    ...
    rsgislib.imageutils.gen_valid_mask(
        input_img, output_img, gdalformat="GTIFF", no_data_val=0.0
    )
...
def test_gen_img_edge_mask(tmp_path):
    ...
    rsgislib.imageutils.gen_img_edge_mask(
        input_img, output_img, gdalformat="GTIFF", n_edge_pxls=10
    )
...
def test_mask_all_band_zero_vals(tmp_path):
    ...
    rsgislib.imageutils.mask_all_band_zero_vals(
        input_img, output_img, gdalformat="GTIFF", out_val=1
    )
...
def test_create_valid_mask(tmp_path):
    ...
    input_ref_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    input_img = os.path.join(tmp_path, "sen2_20210527_aber.tif")
    copy2(input_ref_img, input_img)
    ...
    rsgislib.imageutils.create_valid_mask(
        img_band_info, output_img, gdalformat="GTIFF", tmp_dir=tmp_path
    )
...
def test_subset_to_img(tmp_path):
    ...
    input_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    in_roi_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset.tif")
    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.subset_to_img(
        input_img, in_roi_img, output_img, "GTIFF", rsgislib.TYPE_16UINT
    )
...
def test_subset_to_geoms_bbox(tmp_path):
    ...
    assert len(glob.glob("{}*.tif".format(out_img_base))) == 4
...
def test_subset_pxl_bbox(tmp_path):
    ...
    rsgislib.imageutils.subset_pxl_bbox(
        input_img, output_img, "GTIFF", rsgislib.TYPE_16UINT, 200, 300, 200, 300
    )
...
def test_perform_random_pxl_sample_in_mask(tmp_path):
    ...
    rsgislib.imageutils.perform_random_pxl_sample_in_mask(
        input_img, output_img, gdalformat="GTIFF", mask_vals=1, n_samples=10
    )
    assert os.path.exists(output_img)
...
def test_extract_img_pxl_vals_in_msk():
    ...
    input_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    in_msk_img = os.path.join(DATA_DIR, "aber_osgb_multi_polys_rasters.tif")
...
def test_create_copy_img(tmp_path):
    ...
    output_img = os.path.join(tmp_path, "out_img.tif")
    rsgislib.imageutils.create_copy_img(
        input_img, output_img, 3, 5, "GTIFF", rsgislib.TYPE_32FLOAT
    )
    assert os.path.exists(output_img)
...
def test_create_blank_buf_img_from_ref_img(tmp_path):
    ...
    rsgislib.imageutils.create_blank_buf_img_from_ref_img(
        input_img, output_img, "GTIFF", rsgislib.TYPE_32FLOAT, 50, no_data_val=0
    )
    assert os.path.exists(output_img)
...
def test_create_blank_img_from_ref_vector(tmp_path):
    ...
    rsgislib.imageutils.create_blank_img_from_ref_vector(
        vec_file, vec_lyr, output_img, 20, 4, "GTIFF", rsgislib.TYPE_32FLOAT
    )
    assert os.path.exists(output_img)
...
def test_create_blank_img_from_bbox(tmp_path):
    ...
    rsgislib.imageutils.create_blank_img_from_bbox(
        bbox, wkt_str, output_img, 5, 10, 5, "GTIFF", rsgislib.TYPE_32FLOAT, True
    )
    assert os.path.exists(output_img)
...
def test_create_img_for_each_vec_feat(tmp_path):
    ...
    assert len(glob.glob(os.path.join(tmp_path, "*.tif"))) == 4
...
def test_create_copy_img_vec_extent(tmp_path):
    ...
    rsgislib.imageutils.create_copy_img_vec_extent(
        input_img, vec_file, vec_lyr, output_img, 1, 10, "GTIFF", rsgislib.TYPE_32FLOAT
    )
    assert os.path.exists(output_img)
...
def test_select_img_bands(tmp_path):
    ...
    rsgislib.imageutils.select_img_bands(
        input_img, output_img, "GTIFF", rsgislib.TYPE_16UINT, [1, 2, 3]
    )
    assert os.path.exists(output_img)
...
def test_stack_img_bands(tmp_path):
    ...
    rsgislib.imageutils.stack_img_bands(
        [input_img, input_img], None, output_img, 0, 0, "GTIFF", rsgislib.TYPE_16UINT
    )
    assert os.path.exists(output_img)
...
def test_set_band_names(tmp_path):
    ...
    input_ref_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset_b123_nostats.tif")
    input_img = os.path.join(tmp_path, "sen2_20210527_aber_subset_b123.tif")
    copy2(input_ref_img, input_img)
...
def test_do_images_overlap():
    ...
    input_img = os.path.join(DATA_DIR, "sen2_20210527_aber.tif")
    input_sub_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset_b123.tif")
...
def test_calc_wgs84_pixel_area(tmp_path):
    ...
    rsgislib.imageutils.calc_wgs84_pixel_area(input_img, output_img, gdalformat="GTIFF")
...
def test_calc_wsg84_pixel_size(tmp_path):
    ...
    rsgislib.imageutils.calc_wsg84_pixel_size(input_img, output_img, gdalformat="GTIFF")
...
def test_calc_pixel_locations(tmp_path):
    ...
    rsgislib.imageutils.calc_pixel_locations(input_img, output_img, gdalformat="GTIFF")
...
def test_generate_random_pxl_vals_img(tmp_path):
    ...
    rsgislib.imageutils.generate_random_pxl_vals_img(
        input_img, output_img, gdalformat="GTIFF", low_val=100, up_val=200
    )
...
def test_gen_sampling_grid(tmp_path):
    ...
    rsgislib.imageutils.gen_sampling_grid(
        input_img, output_img, "GTIFF", 10, 10, 100, True
    )
...
def test_whiten_image(tmp_path):
    ...
    output_img = os.path.join(tmp_path, "output_img.tif")
    rsgislib.imageutils.whiten_image(input_img, valid_msk_img, 1, output_img, "GTIFF")
...
def test_spectral_smoothing(tmp_path):
    ...
    valid_msk_img = os.path.join(DATA_DIR, "sen2_20210527_aber_subset_vldmsk.tif")
    output_img = os.path.join(tmp_path, "output_img.tif")
    rsgislib.imageutils.spectral_smoothing(input_img, valid_msk_img, 1, output_img, gdalformat = "GTIFF")
