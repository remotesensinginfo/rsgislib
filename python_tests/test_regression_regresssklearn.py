...
def test_create_search_obj():
    ...
    rsgislib.regression.regresssklearn.create_search_obj(et_obj, et_grid, n_runs=25, n_cv=2, n_cores=1)
...
def test_perform_search_param_opt(tmp_path):
    ...
    et_obj = ExtraTreesRegressor()
    skl_srch_obj = rsgislib.regression.regresssklearn.create_search_obj(et_obj, et_grid, n_runs=25, n_cv=2, n_cores=1)
    ...
    x = rsgislib.vectorattrs.get_vec_cols_as_array(vec_file, vec_lyr, cols=["re_b4", "re_b5", "re_b6", "nir_b7", "nir_b8"])


    opt_params_file = os.path.join(tmp_path, "out.json")
    rsgislib.regression.regresssklearn.perform_search_param_opt(opt_params_file, x, y, skl_srch_obj, data_scaler=None)
...
def test_perform_kfold_fit():
    ...
    y = rsgislib.vectorattrs.get_vec_cols_as_array(vec_file, vec_lyr, cols=["value"])
    x = rsgislib.vectorattrs.get_vec_cols_as_array(vec_file, vec_lyr, cols=["re_b4",
                                                                            "re_b5",
                                                                            "re_b6",
                                                                            "nir_b7",
                                                                            "nir_b8"])
    rsgislib.regression.regresssklearn.perform_kfold_fit(et_obj, x, y, n_splits=2, repeats=2, shuffle=True, data_scaler=None)
...
def test_apply_regress_sklearn_mdl(tmp_path):
    ...
    rsgislib.regression.regresssklearn.apply_regress_sklearn_mdl(et_obj, 1, s2_img, [4,5,6,7,8], vld_msk_img, 1, out_img, gdalformat='KEA', out_band_names=None, calc_stats=True, out_no_date_val=0.0)
