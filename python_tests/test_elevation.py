...
def test_fftDEMFusion(tmp_path):
    ...
    rsgislib.elevation.fftDEMFusion(high_res_dem_img, low_res_dem_img, output_img, c_size)
...
def test_dtmAspectMedianFilter(tmp_path):
    ...
    win_half_size = 10
    gdalformat = "KEA"
    rsgislib.elevation.dtmAspectMedianFilter(input_img, input_aspect_img, output_img, aspect_range, win_half_size, gdalformat)
        
    aspect_med_filter_ref_img = os.path.join(DATA_DIR, "SRTM_aber_aspect_med_filter.kea")
    img_eq, prop_match = rsgislib.imagecalc.areImgsEqual(output_img, aspect_med_filter_ref_img)
    print(prop_match)
    assert img_eq
...
def test_fillDEMSoilleGratin1994(tmp_path):
    ...
    rsgislib.elevation.fillDEMSoilleGratin1994(input_img, valid_mask_img, output_img, 'KEA')
...
def test_shadowmask(tmp_path):
    ...
    max_height = 5000
    gdalformat = "KEA"
    rsgislib.elevation.shadowmask(input_img, output_img, solar_azimuth, solar_zenith, max_height, gdalformat)
...
def test_localIncidenceAngle(tmp_path):
    ...
    solar_zenith = 35.67
    gdalformat = "KEA"
    rsgislib.elevation.localIncidenceAngle(input_img, output_img, solar_azimuth, solar_zenith, gdalformat)
...
def test_localExistanceAngle(tmp_path):
    ...
    solar_zenith = 35.67
    gdalformat = "KEA"
    rsgislib.elevation.localExistanceAngle(input_img, output_img, solar_azimuth, solar_zenith, gdalformat)
...
def test_hillshade(tmp_path):
    ...
    solar_zenith = 35.67
    gdalformat = "KEA"
    rsgislib.elevation.hillshade(input_img, output_img, solar_azimuth, solar_zenith, gdalformat)
