...
class StdImgBlockIter:
    ...
    def __init__(self, img_info_lst, block_size):
        ...
        for img in self.img_info_lst:
            ...
            self.img_info[img.name]["np_arr"] = numpy.zeros(
                (len(img.bands), block_size, block_size), dtype=float
            )
    ...
    def __next__(self):
        ...
        :return: x_block, y_block, img_data (dict with image name as key; each image
                 has a np_arr variable for the image data.
            ...
            for img in self.img_info_lst:
                for band in img.bands:
                    self.img_info[img.name]["bands"][band].ReadAsArray(
                        ...
                        buf_obj=self.img_info[img.name]["np_arr"][band - 1],
                    )
    ...
    def write_block_to_image(self, x_block, y_block, img_blocks):
        ...
        :param img_blocks: A dict of output image block data. Keys must match
            ...
            if img in self.out_imgs_objs:
                ...
                for band in range(block_shp[0]):
                    self.out_imgs_objs[img]["bands"][band + 1].WriteArray(
                        img_blocks[img][band], pxl_x, pxl_y
                    )
