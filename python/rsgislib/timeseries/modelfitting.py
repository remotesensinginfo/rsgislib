...
"""
    ...
    curr_percent = float(info.yblock * info.xtotalblocks + info.xblock) / float(
        info.xtotalblocks * info.ytotalblocks) * 100
...
def get_ST_model_coeffs(json_fp, output_fp, output_driver='KEA', bands=None, num_processes=1, model_type='Lasso',
                        alpha=20, cv=False):
    """
...
:param output_driver: Short driver name for GDAL, e.g. KEA, GTiff.
...
"""
    ...
    # Set up array with the correct output shape
    px_out = numpy.zeros((num_output_bands, info.getBlockSize()[0], info.getBlockSize()[1]), dtype='float64')
